  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-white/20">
        ...
        <div className="container mx-auto max-w-7xl relative z-10 flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          <div className="flex-1 text-center lg:text-left">
            ...
            <h1 className="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-light tracking-tighter leading-[1.1] mb-6">
              ...
              <span className="font-black text-transparent bg-clip-text bg-linear-to-br from-white via-gray-300 to-gray-700">
                Faster.
              </span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
              <Link
                to="/components"
                className="w-full sm:w-auto px-8 py-4 bg-white text-black text-xs md:text-sm uppercase tracking-widest font-bold rounded-sm hover:bg-gray-200 transition-all flex items-center justify-center gap-3 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.15)]"
              >
                Explore Library <FaArrowRight size={12} />
              </Link>
              <Link
                to="/addsnippet"
                className="w-full sm:w-auto px-8 py-4 bg-transparent border border-white/20 text-white text-xs md:text-sm uppercase tracking-widest font-bold rounded-sm hover:bg-white hover:text-black transition-all flex items-center justify-center gap-3 active:scale-95"
              >
          ...
          <div className="flex-1 w-full max-w-lg lg:max-w-none mx-auto lg:mt-0 mt-8">
            <div className="relative group perspective-1000">
              <div className="absolute inset-0 bg-linear-to-r from-blue-500/20 to-purple-500/20 blur-xl transform group-hover:scale-105 transition-transform duration-700 pointer-events-none"></div>
              <div className="bg-[#0a0a0a] border border-white/10 rounded-sm overflow-hidden shadow-2xl relative transform lg:rotate-y-[-5deg] lg:rotate-x-[5deg] transition-transform duration-700 group-hover:rotate-0">
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-white/2">
                  ...
                  <div className="flex gap-2 text-gray-500 relative z-20">
                    <button onClick={handleCopy} className="hover:text-white transition-colors" title="Copy to clipboard">
                ...
                <div className="p-6 font-mono text-[11px] sm:text-xs md:text-sm leading-relaxed text-gray-400 overflow-x-auto bg-black/40 pb-16">
                  <div className="flex"><span className="text-pink-500">import</span>&nbsp;React&nbsp;<span className="text-pink-500">from</span>&nbsp;<span className="text-green-400">'react'</span>;</div>
                  <br />
                  <div className="flex"><span className="text-pink-500">export const</span>&nbsp;<span className="text-blue-400">Button</span>&nbsp;=&nbsp;()&nbsp;<span className="text-pink-500">=&gt;</span>&nbsp;&#123;</div>
                  <div className="flex">&nbsp;&nbsp;<span className="text-pink-500">return</span>&nbsp;(</div>
                  <div className="flex">&nbsp;&nbsp;&nbsp;&nbsp;&lt;<span className="text-blue-400">button</span>&nbsp;<span className="text-yellow-300">className</span>=<span className="text-green-400">"px-6 py-2 bg-white text-black rounded-sm"</span>&gt;</div>
      ...
      <section className="py-24 px-6 bg-[#0a0a0a]">
        <div className="container mx-auto max-w-7xl">
          <div className="text-center mb-16">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 mb-3">Platform Capabilities</h2>
      ...
      <section className="py-32 px-6 relative overflow-hidden bg-[#050505]">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,var(--tw-gradient-stops))] from-white/3 via-transparent to-transparent pointer-events-none"></div>
        <div className="container mx-auto max-w-4xl text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            {!user?._id ? (
              <Link
                to="/register"
                className="w-full sm:w-auto px-12 py-4 bg-white text-black text-xs md:text-sm uppercase tracking-widest font-bold rounded-sm hover:bg-gray-200 transition-all shadow-[0_0_30px_rgba(255,255,255,0.1)] active:scale-95"
              >
                Create Account
              </Link>
            ) : (
...
const StepCard = ({ number, title, desc, onClick }) => (
  <div
    onClick={onClick}
    className="relative z-10 flex flex-col items-center md:items-start text-center md:text-left bg-[#050505] p-8 md:p-10 border border-white/5 rounded-sm hover:border-white/30 cursor-pointer transition-all group"
  >
    <div className="w-14 h-14 bg-[#0a0a0a] border border-white/10 rounded-sm flex items-center justify-center text-lg font-black text-white mb-8 group-hover:bg-white group-hover:text-black transition-colors">
      {number}
    </div>
    <h4 className="text-base md:text-lg font-bold mb-3 tracking-tight uppercase group-hover:text-white transition-colors">{title}</h4>
...
const FeatureCard = ({ icon, title, description, onClick }) => {
  return (
    <div
      onClick={onClick}
      className="p-8 md:p-10 bg-[#050505] border border-white/5 rounded-sm hover:border-white/30 cursor-pointer hover:bg-white/2 transition-all group flex flex-col justify-between h-full"
    >
      <div>
        <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-sm flex items-center justify-center mb-8 group-hover:bg-white transition-all duration-300">
          <div className="text-white group-hover:text-black transition-colors">{icon}</div>
        </div>
        <h3 className="text-sm font-bold uppercase tracking-widest mb-4 group-hover:text-white transition-colors">{title}</h3>
        <p className="text-gray-500 text-xs leading-relaxed">{description}</p>
      </div>
      <div className="mt-8 pt-6 border-t border-white/5">
        <span className="text-[10px] uppercase tracking-widest font-bold text-gray-600 group-hover:text-white transition-colors">Explore Feature →</span>
      </div>
    </div>
  )
}
